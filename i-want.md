...
- A reliable non-interactive `rtk codex exec` review wrapper with enforced timeout and captured final output would help roadmap work; current review invocations can hang for minutes without streaming progress or returning a payload.
- 2026-03-10: The `rtk codex exec` review gap is still active; roadmap-required read-only review passes for the tmux daemon work repeatedly stalled after file reads and did not return a final verdict, so a hard-timeout wrapper plus a guaranteed final-summary capture would remove that blocker.
