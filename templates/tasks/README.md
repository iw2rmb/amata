...
- Ensure every task is the smallest testable slice; if definitive verification steps are unclear, break the task down further or create an enabling task first.
- Document what the task unblocks next and whether enabling work should land before neighbouring stories proceed.
- Update dependency metadata whenever the task changes state so the task index stays accurate.
...
- Populate the **Planned Complexity (COSMIC)** table during drafting so estimates remain linked to their assumptions.
...
1. `implementation-task.md`
   ...
   - **Highlights**: Explicit change list, test coverage requirements, dependency matrices, and cues to record verification evidence plus downstream docs impact in `docs/CHANGELOG.md`.
...
2. `validation-task.md`
   ...
   - **Highlights**: Environment prerequisites, execution steps, artefacts to collect, dependencies, and an entry in `docs/CHANGELOG.md` for repeatable runs.
...
3. `documentation-task.md`
   ...
   - **Highlights**: Audience mapping, affected pages, review steps, dependency callouts (other docs/features), and follow-up enablement checklist.
...
- Populate the Blocked by/Unblocks lists before marking a task "Ready" so other contributors can pick it up confidently.
