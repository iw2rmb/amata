...
- [ ] `relative/task/path.md` (in progress) — task is currently being implemented; only one agent should hold this state.
- Remove the entry once the task is complete so downstream work can advance.

## Queue
- *(Add tasks here in dependency order: unblocked work at the top, most blocked items last.)*
