...
func (r *Runner) recordStepStartedEvent(
	...
	reporter.StepStarted(progressStep(config, flowName, frameID, stepIndex, step, executionLabel, previous, bindings, lookup))
	return snapshot, nil
}
...
func (r *Runner) recordResultEvent(
	...
	reporter.StepFinished(progressResultStep(config, flowName, frameID, step, previous, bindings, result, lookup))
...
func progressStep(config Config, flowName string, frameID string, stepIndex int, step spec.Step, executionLabel string, previous *state.StepResult, bindings map[string]any, lookup func(*state.StepRef) *state.StepResult) progress.Step {
	stepCtx := progressStepContext(config, flowName, frameID, stepIndex, step, executionLabel, previous, bindings, lookup)
...
func progressResultStep(config Config, flowName string, frameID string, step spec.Step, previous *state.StepResult, bindings map[string]any, result state.StepResult, lookup func(*state.StepRef) *state.StepResult) progress.Step {
	stepCtx := progressStepContext(config, flowName, frameID, result.Index, step, "", previous, bindings, lookup)
...
func progressStepContext(config Config, flowName string, frameID string, stepIndex int, step spec.Step, executionLabel string, previous *state.StepResult, bindings map[string]any, lookup func(*state.StepRef) *state.StepResult) executorapi.StepContext {
...
func resumeActiveProgressSteps(config Config, plan *flowPlan, snapshot state.Snapshot) []progress.Step {
	...
	for index := 1; index < len(snapshot.Frames); index++ {
		...
		steps = append(steps, progressStep(config, parentFlowName, snapshot.Frames[index-1].ID, frame.Return.StepIndex, parentFlow.Steps[frame.Return.StepIndex], "", lookup(frame.Previous), snapshot.Frames[index-1].Bindings, lookup))
	}
